...
Plane::Plane(glm::vec3 position, glm::vec2 size, glm::vec3 orientation)
  ...
  vertexArray->addVertexAttributes(Plane::Vertex::vertexAttributes(),
                                   sizeof(Plane::Vertex));
...
void Plane::render(glm::mat4 &mvp, glm::vec3 lightPos, glm::vec3 lightColor, glm::vec3 viewPos) {
...
void Plane::onKeyEvent(int32_t key, int32_t scancode, int32_t action,
                       int32_t mode) {}
