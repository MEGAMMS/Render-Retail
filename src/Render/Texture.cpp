...
void Texture::bufferCubeMapRGBAData(const std::array<std::shared_ptr<const Image>, 6>& images) {
...
std::shared_ptr<const Texture> Texture::loadCubeMapTexture(const std::string& name) {
  ...
  std::shared_ptr<Texture> texture = std::make_shared<Texture>(GL_TEXTURE_CUBE_MAP);
